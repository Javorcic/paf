...
plt.scatter(log_t, log_s, color='pink', label='Mjerenja', zorder=5)
x_line = np.linspace(min(log_t), max(log_t), 200)
plt.plot(x_line, a_log * x_line + b_log, color='palevioletred',
...
plt.figure(figsize=(7, 5))
plt.scatter(t2, s, color='pink', label='Mjerenja', zorder=5)
x_line2 = np.linspace(0, max(t2), 200)
plt.plot(x_line2, a_st2 * x_line2, color='palevioletred',
