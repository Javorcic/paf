...
for ax, T_mjer, L_nom, l_fit, dl_fit, rel_pogr, naziv in [
    ...
    ax.scatter(kut_deg, T_mjer, color='pink', zorder=5, label='Mjerenja')
    ax.plot(np.degrees(theta_plot), period(theta_plot, L_nom), 'rosybrown', label=f'Teorija (L_nom={L_nom*1000:.0f} mm)')
    ax.plot(np.degrees(theta_plot), period(theta_plot, l_fit), 'palevioletred', label=f'Fit: l={l_fit*1000:.1f}±{dl_fit*1000:.1f} mm')
    ax.set_xlabel('Kut θ (°)')
